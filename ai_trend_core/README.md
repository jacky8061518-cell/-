...
只會執行一次「全市場掃描 → 若有異常則啟動 AI 智能體分析 → 顯示報告 → 寫入資料庫」，
跑完就結束，不會像 `main_loop.py` 一樣無限循環等待。適合每天自己挑時間手動檢查市場，
藉此控制 API 呼叫次數與費用。終端機會直接印出：
- 本次掃描到的所有標的之價格、Z-Score、訊號。
- 每個異常標的的完整 AI 分析報告（建議動作、進場 / 停利 / 停損、信心評分、完整推理內容）。
