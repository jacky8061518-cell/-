...
class BuyAndHold:
    ...
    這裡只需要回傳一組不變的等權權重。**引擎負責保證「權重不變就不交易」**
    ——見 ``backtest/engine.py`` 的 ``_rebalance``。這件事一度做反過來：
    早期版本讓引擎每天都用當天價格把權重重新換算一次股數，於是即使權重
    完全沒變，價格一波動就會被迫交易，變成「每日再平衡到固定權重」而不是
    買進持有。用真實資料實測抓到：五檔股票、兩年，錯誤版本跑出 2420 筆
    成交，正確版本是 5 筆（只有建倉當天）。修正後兩者的策略程式碼完全
    不需要記住任何狀態，複雜度回到最初的樣子——問題原本就不在策略這層。
    """
